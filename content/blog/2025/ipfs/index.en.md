...
It's necessary to keep in mind that IPNS-records have a limited time to live (TTL) of 24 hours. To keep it active it's necessary to refresh it before it turns into digital dust.
...
added QmeeLUVdiSTTKQqhWqsffYDtNvvvcTfJdotkNyi1KDEJtQ helloworld.txt
 14 B / 14 B [============================================] 100.00%
 ```
The string `QmeeLUVdiSTTKQqhWqsffYDtNvvvcTfJdotkNyi1KDEJtQ`, returned as output from the `ipfs add ...` command, is the CID of the added file. Now, this CID can be shared—for example, via messenger or email—allowing anyone with the CID to download the file. Downloadable while the computer, serving this file, is turned on, or while the file is cached on other nodes which previously downloaded the file.
...
Interacting with the IPFS network in the way described above is considered canonical, but it's too complicated if all you need is to download a file with a given CID. In this case, the easiest and most convenient way is to use web gateways.
...
IPFS doesn't guarantee file availability if the original node that published the file goes offline. Therefore, it can't be used as a reliable mirror for, say, this blog or any other similar website.
...
[Hosting a static website on IPFS]({{< relref "/blog/2025/hosting-static-web-site-on-ipfs" >}}), which I'm going to describe in the next blog post, is nothing more than a fun toy. Accessing such a site via an external web gateway is painfully slow due to slow content retrieval, so setting up a local gateway is necessary to speed up file delivery. In its turn, this offers no practical advantages over serving a website with a regular web server. Moreover, it's still recommended to place a web server in front of the IPFS web gateway as a reverse proxy.
...
In the case of distributing large files with sudden spikes in demand, IPFS can be useful, but in this scenario, it's no more convenient than BitTorrent.
