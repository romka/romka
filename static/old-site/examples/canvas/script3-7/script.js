...
function draw(context, fog_context) {
  ...
  img_moroz.src = 'moroz-small-2.png'; // ”станавливаем путь к источнику
...
function eraser(e, context, fog_context, radius) {
  ...
  while(i <= elem_count) {
    ...
    if(Math.sqrt(bc * bc + ab * ab) < radius) {
      // ≈сли пиксел попал в круг, то мен€ю его цвет как на нижнем холсте, иначе оставл€ю цвет на такой как на верхнем холсте
